...
% - rows         two columns, written with & and \\ as in a table: the first one is
%                right-aligned, the second one takes the remaining width
%                Each row is typeset on its own, so a box that does not fit on the
%                page is split instead of being pushed to the next one: give every
%                row exactly one &, no optional argument to \\, and put nested
%                tables between braces so their own & and \\ stay hidden
